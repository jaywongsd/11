...
ui_print "——————————————————————————————————————————————————"
#debugUI相机可视化算法日志删除
rm -rf /data/vendor/camera
echo "debugUI相机可视化算法日志删除✅"
...
dumpsys deviceidle whitelist $cust_list

nohup /data/f2fsgc --get="$i" --fstrim=7 --gc="$gc_type" "$file" > /dev/null 2>&1 &
notification_simulation "已清理电池优化名单，后台将进行磁盘维护，手机可能会短暂发热，请耐心等待。" "小羽"
